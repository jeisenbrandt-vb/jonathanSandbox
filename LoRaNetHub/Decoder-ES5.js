...
//===========================================================
var DECODER_MAJOR_VERSION = 2;
var DECODER_MINOR_VERSION = 2;
var DECODER_PATCH_VERSION = 0;
...
function addVoboMetadata(decodedData, fport) {
  ...
  if (portLastDigit == 0 || fport >= 1 && fport < 10) voboType = "VoBoXX";else if (portLastDigit == 1) voboType = "VoBoTC";else if (portLastDigit == 2) voboType = "VoBoXP";
  if (fport == 1) payloadType = "Standard";else if (fport >= 2 && fport <= 9) {
    payloadType = "Modbus Standard";
  } else if (fport == 10) payloadType = "Heartbeat 1.0";else if (fport >= 20 && fport <= 29) {
    ...
    for (var i = 0; i < 16; i++) {
      var valid = payload.data.sensorValid0 >> i & 1;
      if (valid == 1) {
        payload.data.digitalSensorStrings.push(lookupDigitalSensorName(voboType, i));
        var data = payload.data.sensorData0 >> i & 1;
        payload.data.digitalSensorData.push(data);
      }
    }
  } else if (fport >= 60 && fport <= 69) payloadType = "Event Log";else if (fport >= 70 && fport <= 79) payloadType = "Configuration";else if (fport >= 80 && fport <= 89) payloadType = "Notification";else if (fport >= 100 && fport <= 109) payloadType = "Modbus Generic";else if (fport >= 110 && fport <= 119) {
    payloadType = "Analog Input Variable Length";
    for (var _i = 0; _i < payload.data.numOfAinPayloads; _i++) {
      payload.data["analogSensorString" + _i] = lookupAnalogSensorName(voboType, payload.data.ainPayloads[_i].sensorNum0);
      payload.data["engUnitsString" + _i] = lookupUnits(1, payload.data.ainPayloads[_i].sensorUnits0);
    }
...
function bytesToFloat32(bytes) {
    bytes = (bytes[0] << 24)  + (bytes[1] << 16) + (bytes[2] << 8) + (bytes[3]);
...
function parseVoboLibGeneralConfigurationPayload(bytes, fport) {
  ...
  if (decoded.sequenceNumber == 1) {
    ...
    for (var i = 9; i >= 2; i--) {
      byteHex = bytes[i].toString(16).toUpperCase();
      if(byteHex.length == 1) byteHex = "0" + byteHex;
      decoded.joinEUI += byteHex;
...
function readUInt32LE(bytes, offset) {
  ...
  const last = bytes[offset + 3];
  if (first === undefined || last === undefined)
    boundsError(offset, bytes.length - 4);

  return first +
    bytes[++offset] * Math.pow(2, 8) +
    bytes[++offset] * Math.pow(2, 16) +
    last * Math.pow(2, 24);
}
...
function parseVoboLibVoboSyncConfigurationPayload(bytes) {
  ...
  if (decoded.sequenceNumber == 0) {
    ...
    decoded.vbsTimeReference = readUInt32LE(bytes.slice(3, 7), 0); // VoboSync Time Reference (32-bit)
...
function parseVoboTCGeneralConfigurationPayload(bytes) {
  ...
  if (decoded.sequenceNumber == 0) {
    ...
    decoded.tcSensorsTransmitEnable = Boolean((bytes[10] & 0x20) >> 5);
    ; // TC Sensors Transmit Enable (1-bit)
...
function parseVoboXPGeneralConfigurationPayload(bytes) {
  ...
  if (decoded.sequenceNumber == 1) {
    ...
    decoded.serialProtoRS232 = bytes[5] >> 2 & 0x01; // Serial Protocol for RS232 (1-bit)
  ...
  while (byteIdx < payloadSize - 1) {
    ...
    if (groupIdx > 0) {
      ...
      if (blockType == 0x00)
        ...
        // Non fragmented block
        {
          var registersValues = [];
          var numOfRegisters = bytes[byteIdx] & 0x7F;
          byteIdx++;
          for (var regIdx = 0; regIdx < numOfRegisters; regIdx++) {
            var registerValue = bytes[byteIdx + 1] << 8 | bytes[byteIdx];
        ...
        // Fragmented block
        {
          ...
          for (var _regIdx = 0; _regIdx < numOfRegToRead; _regIdx++) {
            var registerValue = bytes[byteIdx + 1] << 8 | bytes[byteIdx];
            decoded["group" + groupIdx + "register" + (registerOffset + _regIdx + 1)] = registerValue;
            registersValues.push(registerValue);
            byteIdx += 2;
          }
        }
...
function lookupAnalogSensorName(voboType, sensorNum) {
  var analogSensorsTableXX = [{
    "Analog Sensor Number": "0",
    "Analog Sensor Name": "Battery Voltage"
  }, {
    "Analog Sensor Number": "1",
    "Analog Sensor Name": "AIN1"
  }, {
    "Analog Sensor Number": "2",
    "Analog Sensor Name": "AIN2"
  }, {
    "Analog Sensor Number": "3",
    "Analog Sensor Name": "AIN3"
  }, {
    "Analog Sensor Number": "15",
    "Analog Sensor Name": "ADC Temperature"
  }];
  var analogSensorsTableTC = [{
    "Analog Sensor Number": "0",
    "Analog Sensor Name": "Battery Voltage"
  }, {
    "Analog Sensor Number": "1",
    "Analog Sensor Name": "TC1"
  }, {
    "Analog Sensor Number": "2",
    "Analog Sensor Name": "TC2"
  }, {
    "Analog Sensor Number": "3",
    "Analog Sensor Name": "TC3"
  }, {
    "Analog Sensor Number": "4",
    "Analog Sensor Name": "TC4"
  }, {
    "Analog Sensor Number": "5",
    "Analog Sensor Name": "TC5"
  }, {
    "Analog Sensor Number": "6",
    "Analog Sensor Name": "TC6"
  }, {
    "Analog Sensor Number": "7",
    "Analog Sensor Name": "TC7"
  }, {
    "Analog Sensor Number": "8",
    "Analog Sensor Name": "TC8"
  }, {
    "Analog Sensor Number": "9",
    "Analog Sensor Name": "TC9"
  }, {
    "Analog Sensor Number": "10",
    "Analog Sensor Name": "TC10"
  }, {
    "Analog Sensor Number": "11",
    "Analog Sensor Name": "TC11"
  }, {
    "Analog Sensor Number": "12",
    "Analog Sensor Name": "TC12"
  }, {
    "Analog Sensor Number": "13",
    "Analog Sensor Name": "Cold Joint Temperature"
  }];
  var analogSensorsTableXP = [{
    "Analog Sensor Number": "0",
    "Analog Sensor Name": "3.3V Supply Voltage"
  }, {
    "Analog Sensor Number": "1",
    "Analog Sensor Name": "AIN1"
  }, {
    ...
    "Analog Sensor Number": "4",
    "Analog Sensor Name": "DIN1"
  }, {
    "Analog Sensor Number": "5",
    "Analog Sensor Name": "DIN1 Pulse Count"
  }, {
    "Analog Sensor Number": "6",
    "Analog Sensor Name": "DIN2"
  }, {
    "Analog Sensor Number": "7",
    "Analog Sensor Name": "DIN2 Pulse Count"
  }, {
    "Analog Sensor Number": "8",
    "Analog Sensor Name": "RLY1 Voltage"
  }, {
    "Analog Sensor Number": "9",
    "Analog Sensor Name": "RLY2 Voltage"
  }, {
    "Analog Sensor Number": "10",
    "Analog Sensor Name": "RLY3 Voltage"
  }, {
    "Analog Sensor Number": "11",
    "Analog Sensor Name": "RLY4 Voltage"
  }, {
    "Analog Sensor Number": "12",
    "Analog Sensor Name": "WKUP"
  }, {
    "Analog Sensor Number": "13",
    "Analog Sensor Name": "VPP Voltage"
  }, {
    "Analog Sensor Number": "14",
    "Analog Sensor Name": "VIN Voltage"
  }, {
    "Analog Sensor Number": "15",
    "Analog Sensor Name": "ADC Temperature"
  }, {
    "Analog Sensor Number": "16",
    "Analog Sensor Name": "Cont Meas Period"
  }, {
    "Analog Sensor Number": "17",
    "Analog Sensor Name": "Cont Meas Count"
  }];
  ...
  if (voboType == "VoBoXX") {
    ...
    if (sensorNumEncoding == 0x01) analogSensorNameSuffix = " (Max)";else if (sensorNumEncoding == 0x02) analogSensorNameSuffix = " (Min)";else if (sensorNumEncoding == 0x03) analogSensorNameSuffix = " (Avg)";
  ...
  var analogSensorRow = [];
  for (let index = 0; index < analogSensorsTable.length; index++) {
    const element = analogSensorsTable[index];
    if(element["Analog Sensor Number"] == sensorNum.toString()) analogSensorRow = element;
  }
  if(analogSensorRow.length == 0) {
...
function lookupDigitalSensorName(voboType, sensorNum) {
  var digitalSensorsTableXX = [{
    "Digital Sensor Number": "0",
    "Digital Sensor Name": "WKUP"
  }, {
    "Digital Sensor Number": "1",
    "Digital Sensor Name": "DIN1"
  }, {
    "Digital Sensor Number": "2",
    "Digital Sensor Name": "DIN2"
  }, {
    "Digital Sensor Number": "3",
    "Digital Sensor Name": "DIN3"
  }];
  var digitalSensorsTableTC = [{
    "Digital Sensor Number": "0",
    "Digital Sensor Name": "WKUP"
  }];
  var digitalSensorsTableXP = [{
    "Digital Sensor Number": "0",
    "Digital Sensor Name": "WKUP"
  }, {
    "Digital Sensor Number": "1",
  ...
  var digitalSensorRow = [];
  for (let index = 0; index < digitalSensorsTable.length; index++) {
    const element = digitalSensorsTable[index];
    if(element["Digital Sensor Number"] == sensorNum.toString()) digitalSensorRow = element;
  }
  if(digitalSensorRow.length == 0) {
...
function lookupUnits(outputType, unitCode) {
  var engUnitsTable = [{
    "Units Code": "1",
    "Description": "inches of water at 20 degC (68 degF)",
    "Abbreviated Units": "inH2O (20 degC or 68 degF)"
  }, {
    "Units Code": "2",
    "Description": "inches of mercury at 0 degC (32 degF)",
    "Abbreviated Units": "inHg (20 degC or 68 degF)"
  }, {
    "Units Code": "3",
    "Description": "feet of water at 20 degC (68 degF)",
    "Abbreviated Units": "ftH2O (20 degC or 68 degF)"
  }, {
    "Units Code": "4",
    "Description": "millimeters of water at 20 degC (68 degF)",
    "Abbreviated Units": "mmH2O (20 degC or 68 degF)"
  }, {
    "Units Code": "5",
    "Description": "millimeters of mercury at 0 degC (32 degF)",
    "Abbreviated Units": "mmHg (0 degC or 32 degF)"
  }, {
    "Units Code": "6",
    "Description": "pounds per square inch",
    "Abbreviated Units": "psi"
  }, {
    "Units Code": "7",
    "Description": "bars",
    "Abbreviated Units": "bar"
  }, {
    "Units Code": "8",
    "Description": "millibars",
    "Abbreviated Units": "mbar"
  }, {
    "Units Code": "9",
    "Description": "grams per square centimeter",
    "Abbreviated Units": "g/cm^2"
  }, {
    "Units Code": "10",
    "Description": "kilograms per square centimeter",
    "Abbreviated Units": "kg/cm^2"
  }, {
    "Units Code": "11",
    "Description": "pascals",
    "Abbreviated Units": "Pa"
  }, {
    "Units Code": "12",
    "Description": "kilopascals",
    "Abbreviated Units": "kPa"
  }, {
    "Units Code": "13",
    "Description": "torr",
    "Abbreviated Units": "torr"
  }, {
    "Units Code": "14",
    "Description": "atmospheres",
    "Abbreviated Units": "atm"
  }, {
    "Units Code": "15",
    "Description": "cubic feet per minute",
    "Abbreviated Units": "ft^3/min"
  }, {
    "Units Code": "16",
    "Description": "gallons per minute",
    "Abbreviated Units": "usg/min"
  }, {
    "Units Code": "17",
    "Description": "liters per minute",
    "Abbreviated Units": "L/min"
  }, {
    "Units Code": "18",
    "Description": "imperial gallons per minute",
    "Abbreviated Units": "impgal/min"
  }, {
    "Units Code": "19",
    "Description": "cubic meter per hour",
    "Abbreviated Units": "m^3/h"
  }, {
    "Units Code": "20",
    "Description": "feet per second",
    "Abbreviated Units": "ft/s"
  }, {
    "Units Code": "21",
    "Description": "meters per seond",
    "Abbreviated Units": "m/s"
  }, {
    "Units Code": "22",
    "Description": "gallons per second",
    "Abbreviated Units": "usg/s"
  }, {
    "Units Code": "23",
    "Description": "million gallons per day",
    "Abbreviated Units": "Musg/d"
  }, {
    "Units Code": "24",
    "Description": "liters per second",
    "Abbreviated Units": "L/s"
  }, {
    "Units Code": "25",
    "Description": "million liters per day",
    "Abbreviated Units": "ML/d"
  }, {
    "Units Code": "26",
    "Description": "cubic feet per second",
    "Abbreviated Units": "ft^3/s"
  }, {
    "Units Code": "27",
    "Description": "cubic feet per day",
    "Abbreviated Units": "ft^3/d"
  }, {
    "Units Code": "28",
    "Description": "cubic meters per second",
    "Abbreviated Units": "m^3/s"
  }, {
    "Units Code": "29",
    "Description": "cubic meters per day",
    "Abbreviated Units": "m^3/d"
  }, {
    "Units Code": "30",
    "Description": "imperial gallons per hour",
    "Abbreviated Units": "impgal/h"
  }, {
    "Units Code": "31",
    "Description": "imperial gallons per day",
    "Abbreviated Units": "impgal/d"
  }, {
    "Units Code": "32",
    ...
    "Abbreviated Units": "mV"
  }, {
    "Units Code": "37",
    "Description": "ohms",
    "Abbreviated Units": "ohm"
  }, {
    "Units Code": "38",
    "Description": "hertz",
    "Abbreviated Units": "hz"
  }, {
    "Units Code": "39",
    "Description": "milliamperes",
    "Abbreviated Units": "mA"
  }, {
    "Units Code": "40",
    "Description": "gallons",
    "Abbreviated Units": "usg"
  }, {
    "Units Code": "41",
    "Description": "liters",
    "Abbreviated Units": "L"
    ...
    "Units Code": "42",
    "Description": "imperial gallons",
    "Abbreviated Units": "impgal"
  }, {
    "Units Code": "43",
    "Description": "cubic meters",
    "Abbreviated Units": "m^3"
  }, {
    "Units Code": "44",
    "Description": "feet",
    "Abbreviated Units": "ft"
  }, {
    "Units Code": "45",
    "Description": "meters",
    "Abbreviated Units": "m"
    ...
    "Units Code": "46",
    "Description": "barrels",
    "Abbreviated Units": "bbl"
  }, {
    "Units Code": "47",
    "Description": "inches",
    "Abbreviated Units": "in"
  }, {
    "Units Code": "48",
    "Description": "centimeters",
    "Abbreviated Units": "cm"
  }, {
    "Units Code": "49",
    "Description": "millimeters",
    "Abbreviated Units": "mm"
  }, {
    "Units Code": "50",
    "Description": "minutes",
    "Abbreviated Units": "min"
  }, {
    "Units Code": "51",
    "Description": "seconds",
    "Abbreviated Units": "s"
    ...
    "Abbreviated Units": "centistokes"
  }, {
    "Units Code": "55",
    "Description": "centipoise",
    "Abbreviated Units": "cP"
  }, {
    "Units Code": "56",
    "Description": "microsiemens",
    "Abbreviated Units": "microsiemens"
  }, {
    "Units Code": "57",
    "Description": "percent",
    "Abbreviated Units": "%"
    ...
    "Units Code": "63",
    "Description": "pounds",
    "Abbreviated Units": "lb"
  }, {
    "Units Code": "64",
    "Description": "short tons",
    "Abbreviated Units": "short ton"
  }, {
    "Units Code": "65",
    "Description": "long tons",
    "Abbreviated Units": "long ton"
  }, {
    "Units Code": "66",
    "Description": "millisiemens per centimeter",
    "Abbreviated Units": "millisiemens/cm"
  }, {
    "Units Code": "67",
    "Description": "microsiemens per centimeter",
    "Abbreviated Units": "microsiemens/cm"
  }, {
    "Units Code": "68",
    "Description": "newton",
    "Abbreviated Units": "N"
    ...
    "Abbreviated Units": "N m"
  }, {
    "Units Code": "70",
    "Description": "grams per second",
    "Abbreviated Units": "g/s"
  }, {
    "Units Code": "71",
    "Description": "grams per minute",
    "Abbreviated Units": "g/min"
  }, {
    "Units Code": "72",
    "Description": "grams per hour",
    "Abbreviated Units": "g/h"
  }, {
    "Units Code": "73",
    "Description": "kilograms per second",
    "Abbreviated Units": "kg/s"
  }, {
    "Units Code": "74",
    "Description": "kilograms per minute",
    "Abbreviated Units": "kg/min"
  }, {
    "Units Code": "75",
    "Description": "kilograms per hour",
    "Abbreviated Units": "kg/h"
  }, {
    "Units Code": "76",
    "Description": "kilograms per day",
    "Abbreviated Units": "kg/d"
  }, {
    "Units Code": "77",
    "Description": "metric tons per minute",
    "Abbreviated Units": "t/min"
  }, {
    "Units Code": "78",
    "Description": "metric tons per hour",
    "Abbreviated Units": "t/h"
  }, {
    "Units Code": "79",
    "Description": "metric tons per day",
    "Abbreviated Units": "t/d"
  }, {
    "Units Code": "80",
    "Description": "pounds per second",
    "Abbreviated Units": "lb/s"
  }, {
    "Units Code": "81",
    "Description": "pounds per minute",
    "Abbreviated Units": "lb/min"
  }, {
    "Units Code": "82",
    "Description": "pounds per hour",
    "Abbreviated Units": "lb/h"
  }, {
    "Units Code": "83",
    "Description": "pounds per day",
    "Abbreviated Units": "lb/d"
  }, {
    "Units Code": "84",
    "Description": "short tons per minute",
    "Abbreviated Units": "short ton/min"
  }, {
    "Units Code": "85",
    "Description": "short tons per hour",
    "Abbreviated Units": "short ton/h"
  }, {
    "Units Code": "86",
    "Description": "short tons per day",
    "Abbreviated Units": "short ton/d"
  }, {
    "Units Code": "87",
    "Description": "long tons per hour",
    "Abbreviated Units": "long ton/h"
  }, {
    "Units Code": "88",
    "Description": "long tons per day",
    "Abbreviated Units": "long ton/d"
  }, {
    "Units Code": "89",
    "Description": "deka therm",
    "Abbreviated Units": "Dth"
  }, {
    "Units Code": "90",
    "Description": "specific gravity units",
    "Abbreviated Units": "specific gravity units"
  }, {
    "Units Code": "91",
    "Description": "grams per cubic centimeter",
    "Abbreviated Units": "g/cm^3"
  }, {
    "Units Code": "92",
    "Description": "kilograms per cubic meter",
    "Abbreviated Units": "kg/m^3"
  }, {
    "Units Code": "93",
    "Description": "pounds per gallon",
    "Abbreviated Units": "lb/usg"
  }, {
    "Units Code": "94",
    "Description": "pounds per cubic feet",
    "Abbreviated Units": "lb/ft^3"
  }, {
    "Units Code": "95",
    "Description": "grams per milliliter",
    "Abbreviated Units": "g/mL"
  }, {
    "Units Code": "96",
    "Description": "kilograms per liter",
    "Abbreviated Units": "kg/L"
  }, {
    "Units Code": "97",
    "Description": "grams per liter",
    "Abbreviated Units": "g/L"
  }, {
    "Units Code": "98",
    "Description": "pounds per cubic inch",
    "Abbreviated Units": "lb/in^3"
  }, {
    "Units Code": "99",
    "Description": "short tons per cubic yard",
    "Abbreviated Units": "short ton/yd^3"
  }, {
    "Units Code": "100",
    "Description": "degrees twaddell",
    "Abbreviated Units": "degTw"
  }, {
    "Units Code": "101",
    "Description": "degrees brix",
    "Abbreviated Units": "degBx"
  }, {
    "Units Code": "102",
    "Description": "degrees baume heavy",
    "Abbreviated Units": "BH"
  }, {
    "Units Code": "103",
    "Description": "degrees baume light",
    "Abbreviated Units": "BL"
  }, {
    "Units Code": "104",
    "Description": "degrees API",
    "Abbreviated Units": "degAPI"
  }, {
    "Units Code": "105",
    "Description": "percent solids per weight",
    "Abbreviated Units": "% solid/weight"
  }, {
    "Units Code": "106",
    "Description": "percent solids per volume",
    "Abbreviated Units": "% solid/volume"
  }, {
    "Units Code": "107",
    "Description": "degrees balling",
    "Abbreviated Units": "degrees balling"
  }, {
    "Units Code": "108",
    "Description": "proof per volume",
    "Abbreviated Units": "proof/volume"
  }, {
    "Units Code": "109",
    "Description": "proof per mass",
    "Abbreviated Units": "proof/mass"
  }, {
    "Units Code": "110",
    "Description": "bushels",
    "Abbreviated Units": "bushel"
  }, {
    "Units Code": "111",
    "Description": "cubic yards",
    "Abbreviated Units": "yd^3"
  }, {
    "Units Code": "112",
    "Description": "cubic feet",
    "Abbreviated Units": "ft^3"
  }, {
    "Units Code": "113",
    "Description": "cubic inches",
    "Abbreviated Units": "in^3"
  }, {
    "Units Code": "114",
    "Description": "inches per second",
    "Abbreviated Units": "in/s"
  }, {
    "Units Code": "115",
    "Description": "inches per minute",
    "Abbreviated Units": "in/min"
  }, {
    "Units Code": "116",
    "Description": "feet per minute",
    "Abbreviated Units": "ft/min"
  }, {
    "Units Code": "117",
    "Description": "degrees per second",
    "Abbreviated Units": "deg/s"
  }, {
    "Units Code": "118",
    "Description": "revolutions per second",
    "Abbreviated Units": "rev/s"
  }, {
    "Units Code": "119",
    "Description": "revolutions per minute",
    "Abbreviated Units": "rpm"
  }, {
    "Units Code": "120",
    "Description": "meters per hour",
    "Abbreviated Units": "m/hr"
  }, {
    "Units Code": "121",
    "Description": "normal cubic meters per hour",
    "Abbreviated Units": "normal m^3/h"
  }, {
    "Units Code": "122",
    "Description": "normal liter per hour",
    "Abbreviated Units": "normal L/h"
  }, {
    "Units Code": "123",
    "Description": "standard cubic feet per minute",
    "Abbreviated Units": "standard ft^3/min"
  }, {
    "Units Code": "124",
    "Description": "bbl liq",
    "Abbreviated Units": "bbl liq"
  }, {
    "Units Code": "125",
    "Description": "ounce",
    "Abbreviated Units": "oz"
  }, {
    "Units Code": "126",
    "Description": "foot pound force",
    "Abbreviated Units": "ft lb force"
  }, {
    "Units Code": "127",
    "Description": "kilowatt",
    "Abbreviated Units": "kW"
  }, {
    "Units Code": "128",
    "Description": "kilowatt hour",
    "Abbreviated Units": "KWh"
  }, {
    "Units Code": "129",
    "Description": "horsepower",
    "Abbreviated Units": "hp"
  }, {
    "Units Code": "130",
    "Description": "cubic feet per hour",
    "Abbreviated Units": "ft^3/h"
  }, {
    "Units Code": "131",
    "Description": "cubic meters per minute",
    "Abbreviated Units": "m^3/min"
  }, {
    "Units Code": "132",
    "Description": "barrels per second",
    "Abbreviated Units": "bbl/s"
  }, {
    "Units Code": "133",
    "Description": "barrels per minute",
    "Abbreviated Units": "bbl/min"
  }, {
    "Units Code": "134",
    "Description": "barrels per hour",
    "Abbreviated Units": "bbl/h"
  }, {
    "Units Code": "135",
    "Description": "barrels per day",
    "Abbreviated Units": "bbl/d"
  }, {
    "Units Code": "136",
    "Description": "gallons per hour",
    "Abbreviated Units": "usg/h"
  }, {
    "Units Code": "137",
    "Description": "imperial gallons per second",
    "Abbreviated Units": "impgal/s"
  }, {
    "Units Code": "138",
    "Description": "liters per hour",
    "Abbreviated Units": "L/h"
  }, {
    "Units Code": "139",
    "Description": "parts per million",
    "Abbreviated Units": "ppm"
  }, {
    "Units Code": "140",
    "Description": "megacalorie per hour",
    "Abbreviated Units": "Mcal/h"
  }, {
    "Units Code": "141",
    "Description": "megajoule per hour",
    "Abbreviated Units": "MJ/h"
  }, {
    "Units Code": "142",
    "Description": "British thermal unit per hour",
    "Abbreviated Units": "BTU/h"
  }, {
    "Units Code": "143",
    "Description": "degrees",
    "Abbreviated Units": "degrees"
  }, {
    "Units Code": "144",
    "Description": "radian",
    "Abbreviated Units": "rad"
  }, {
    "Units Code": "145",
    "Description": "inches of water at 15.6 degC (60 degF)",
    "Abbreviated Units": "inH20 (15.6 degC or 60 degF)"
  }, {
    "Units Code": "146",
    "Description": "micrograms per liter",
    "Abbreviated Units": "micrograms/L"
  }, {
    "Units Code": "147",
    "Description": "micrograms per cubic meter",
    "Abbreviated Units": "micrograms/m^3"
  }, {
    "Units Code": "148",
    "Description": "percent consitency",
    "Abbreviated Units": "% consistency"
  }, {
    "Units Code": "149",
    "Description": "volume percent",
    "Abbreviated Units": "volume %"
  }, {
    "Units Code": "150",
    "Description": "percent steam quality",
    "Abbreviated Units": "% steam quality"
  }, {
    "Units Code": "151",
    "Description": "feet in sixteenths",
    "Abbreviated Units": "ft in sixteenths"
  }, {
    "Units Code": "152",
    "Description": "cubic feet per pound",
    "Abbreviated Units": "ft^3/lb"
  }, {
    "Units Code": "153",
    "Description": "picofarads",
    "Abbreviated Units": "pF"
  }, {
    "Units Code": "154",
    "Description": "milliliters per liter",
    "Abbreviated Units": "mL/L"
  }, {
    "Units Code": "155",
    "Description": "microliters per liter",
    "Abbreviated Units": "microliters/L"
  }, {
    "Units Code": "156",
    "Description": "percent plato",
    "Abbreviated Units": "% plato"
  }, {
    "Units Code": "157",
    "Description": "percent lower explosion level",
    "Abbreviated Units": "% lower explosion level"
  }, {
    "Units Code": "158",
    "Description": "mega calorie",
    "Abbreviated Units": "Mcal"
  }, {
    "Units Code": "159",
    "Description": "kiloohms",
    "Abbreviated Units": "kohm"
  }, {
    "Units Code": "160",
    "Description": "megajoule",
    "Abbreviated Units": "MJ"
  }, {
    "Units Code": "161",
    "Description": "British thermal unit",
    "Abbreviated Units": "BTU"
  }, {
    "Units Code": "162",
    "Description": "normal cubic meter",
    "Abbreviated Units": "normal m^3"
  }, {
    "Units Code": "163",
    "Description": "normal liter",
    "Abbreviated Units": "normal L"
  }, {
    "Units Code": "164",
    "Description": "standard cubic feet",
    "Abbreviated Units": "normal ft^3"
  }, {
    "Units Code": "165",
    "Description": "parts per billion",
    "Abbreviated Units": "parts/billion"
  }, {
    "Units Code": "166",
    "Description": "ampere",
    "Abbreviated Units": "A"
    ...
    "Units Code": "167",
    "Description": "millimeters per meter",
    "Abbreviated Units": "mm/m"
  }, {
    "Units Code": "168",
    "Description": "seconds since epoch",
    "Abbreviated Units": "epoch sec"
  }, {
    "Units Code": "169",
    "Description": "no units",
    "Abbreviated Units": "no units"
  }, {
    "Units Code": "170",
    ...
    "Abbreviated Units": "khz"
  }, {
    "Units Code": "235",
    "Description": "gallons per day",
    "Abbreviated Units": "usg/d"
  }, {
    "Units Code": "236",
    "Description": "hectoliters",
    "Abbreviated Units": "hL"
  }, {
    "Units Code": "237",
    "Description": "megapascals",
    "Abbreviated Units": "MPa"
  }, {
    "Units Code": "238",
    "Description": "inches of water at 4 degC (39.2 degF)",
    "Abbreviated Units": "inH2O (4 degC or 39.2 degF)"
  }, {
    "Units Code": "239",
    "Description": "millimeters of water at 4 degC (39.2 degF)",
    "Abbreviated Units": "mmH2O (4 degC or 39.2 degF)"
  }, {
    "Units Code": "253",
    "Description": "edge count or pulse count",
    "Abbreviated Units": "counts"
  }, {
    "Units Code": "254",
    "Description": "raw ADC code",
    "Abbreviated Units": "ADC code"
  }, {
    "Units Code": "255",
    "Description": "data not valid",
    "Abbreviated Units": "data not valid"
  }];
  var outputString = "";
  var engUnitsRow = [];
  for (let index = 0; index < engUnitsTable.length; index++) {
    const element = engUnitsTable[index];
    if(element["Units Code"] == unitCode.toString()) engUnitsRow = element;
  }
  if(engUnitsRow.length == 0) {
...
function printPayload(payload) {
  if (payload.data.payloadType == "Standard") {
    var modbusSlot0HexString = payload.data.Modbus0.toString(16).toUpperCase().padStart(4, 0);
    console.log("%s | %s | %s | %s | " + "DIN1 = %d | DIN2 = %d | DIN3 = %d | WKUP = %d | ADC1 = %d | ADC2 = %d | ADC3 = %d | " + "Battery = %d | Temperature = %f | Modbus0 = 0x%s  |\n", payload.timestamp, payload.deveui, payload.data.voboType, payload.data.payloadType, payload.data.DIN1, payload.data.DIN2, payload.data.DIN3, payload.data.WKUP, payload.data.ADC1, payload.data.ADC2, payload.data.ADC3, payload.data.Battery, payload.data.Temperature, modbusSlot0HexString);
    return;
  }
  if (payload.data.payloadType == "Modbus Standard") {
    dataKeys = Object.keys(payload.data);
    var _stringToPrint = payload.timestamp + " | " + payload.deveui + " | " + payload.data.voboType + " | " + payload.data.payloadType + " | ";
    for (var idx = 0; idx < dataKeys.length; idx++) {
      if (dataKeys[idx] != "fport" && dataKeys[idx] != "voboType" && dataKeys[idx] != "payloadType") {
        var fieldName = dataKeys[idx];
        var valueHex = "0x" + payload.data[fieldName].toString(16).toUpperCase().padStart(4, 0);
        _stringToPrint += fieldName + " = " + valueHex + " | ";
      }
    }
    console.log("%s\n", _stringToPrint);
    return;
  ...
  if (payload.data.payloadType == "Heartbeat 1.0") {
    var firmwareRevisionHexString = payload.data.fwVersionMajor.toString(10) + "." + payload.data.fwVersionMinor.toString(10) + "." + payload.data.fwVersionPatch.toString(10) + "." + payload.data.fwVersionCustom.toString(10);
    console.log("%s | %s | %s | %s | " + "Battery Level MV = %d | FW Version = %s | Last RSSI = %d | Analog Voltage Config = %d | Analog Power Time Config = %d | Failed Transmissions Before Rejoin Config = %d | " + "Cycle Through FSB Config = %d | ACK Enable Config = %d | ACK Frequency Config = %d | ACK Request Config = %d | Battery Level Threshold Config = %d |\n", payload.timestamp, payload.deveui, payload.data.voboType, payload.data.payloadType, payload.data.batteryLevelMV, firmwareRevisionHexString, payload.data.recSignalLevels, payload.data.analogVoltageConfig, payload.data.analogPowerTimeConfig, payload.data.failedTransmissionBeforeRejoinConfig, payload.data.cycleThroughFSB, payload.data.ackEnable, payload.data.ackFreq, payload.data.ackReq, payload.data.batteryLevelThreshold);
    return;
  }
  if (payload.data.payloadType == "Heartbeat 2.0") {
    var _firmwareRevisionHexString = payload.data.firmwareRevision.toString(16).toUpperCase().padStart(3, 0);
    console.log("%s | %s | %s | %s | " + "Battery Level = %d | Fatal Errors Total = %d | RSSI Avg = %d | Failed Join Attempts Total = %d | Config Update Occurred = %d | Firmware Revision = 0x%s | " + "Reboots Total = %d | Failed Transmits Total = %d | Error Event Logs Total = %d | Warning Event Logs Total = %d | Info Event Logs Total = %d | Measurement Packets Total = %d |\n", payload.timestamp, payload.deveui, payload.data.voboType, payload.data.payloadType, payload.data.batteryLevel, payload.data.fatalErrorsTotal, payload.data.rssiAvg, payload.data.failedJoinAttemptsTotal, payload.data.configUpdateOccurred, _firmwareRevisionHexString, payload.data.rebootsTotal, payload.data.failedTransmitsTotal, payload.data.errorEventLogsTotal, payload.data.warningEventLogsTotal, payload.data.infoEventLogsTotal, payload.data.measurementPacketsTotal);
    return;
  }
  if (payload.data.payloadType == "One Analog Input") {
    var fractionDigitsData0 = 1;
  ...
  if (payload.data.payloadType == "Two Analog Inputs") {
    var _fractionDigitsData = 1;
    if (payload.data.engUnitsString0 == "mV" || payload.data.engUnitsString0 == "V" || payload.data.engUnitsString0 == "mA" || payload.data.engUnitsString0 == "A") _fractionDigitsData = 3;
    if (payload.data.engUnitsString0 == "ADC code") _fractionDigitsData = 0;
    var fractionDigitsData1 = 1;
    ...
    console.log("%s | %s | %s | %s | %s = %f %s | %s = %f %s |\n", payload.timestamp, payload.deveui, payload.data.voboType, payload.data.payloadType, payload.data.analogSensorString0, parseFloat(payload.data.sensorData0).toFixed(_fractionDigitsData), payload.data.engUnitsString0, payload.data.analogSensorString1, parseFloat(payload.data.sensorData1).toFixed(fractionDigitsData1), payload.data.engUnitsString1);
    return;
  }
  if (payload.data.payloadType == "Analog Input Variable Length") {
    var stringToPrint = "";
    for (var i = 0; i < payload.data.numOfAinPayloads; i++) {
  ...
  if (payload.data.payloadType == "Modbus Standard Variable Length") {
    ...
    var _stringToPrint2 = payload.timestamp + " | " + payload.deveui + " | " + payload.data.voboType + " | " + payload.data.payloadType + " | ";
    for (var _idx = 0; _idx < dataKeys.length; _idx++) {
      var _fieldName = dataKeys[_idx];
      var _valueHex = "0x" + payload.data.modbusSlots[_fieldName].toString(16).toUpperCase().padStart(4, 0);
      _stringToPrint2 += _fieldName + " = " + _valueHex + " | ";
    }
    console.log("%s\n", _stringToPrint2);
    return;
  }
  if (payload.data.payloadType == "Digital Inputs") {
    var stringToPrint = "";
    for (var _i = 0; _i < payload.data.digitalSensorData.length; _i++) {
      stringToPrint = stringToPrint + payload.data.digitalSensorStrings[_i] + " = " + payload.data.digitalSensorData[_i].toString() + " | ";
    }
    console.log("%s | %s | %s | %s | %s\n", payload.timestamp, payload.deveui, payload.data.voboType, payload.data.payloadType, stringToPrint);
    return;
  }
  if (payload.data.payloadType == "Event Log") {
    var eventTimestampDateTime = new Date(payload.data.eventTimestamp * 1000).toISOString();
    var eventCodeHexString = payload.data.eventCode.toString(16).toUpperCase().padStart(4, 0);
    var metadataHexString = Buffer.from(payload.data.metadata).toString("hex").toUpperCase();
  ...
  if (payload.data.payloadType == "Modbus Generic") {
    var _dataKeys = Object.keys(payload.data);
    var _stringToPrint3 = payload.timestamp + " | " + payload.deveui + " | " + payload.data.voboType + " | " + payload.data.payloadType + " | ";
    for (var _idx2 = 0; _idx2 < _dataKeys.length; _idx2++) {
      if (_dataKeys[_idx2] != "fport" && _dataKeys[_idx2] != "voboType" && _dataKeys[_idx2] != "payloadType") {
        var _fieldName2 = _dataKeys[_idx2];
        var regValueHexStr = "0x" + payload.data[_fieldName2].toString(16).toUpperCase().padStart(4, 0);
        _stringToPrint3 += _fieldName2 + " = " + regValueHexStr + " | ";
      }
    }
    console.log("%s\n", _stringToPrint3);
    return;
